...
class ClassificationErrorDTOMapperTest {

  @Test
  void whenMapThenReturnPuErrorDTO() {
    // Given
    ClassificationErrorDTO errorDTO = TestUtils.getPodamFactory().manufacturePojo(ClassificationErrorDTO.class);
    Objects.requireNonNull(errorDTO.getFields());

    // When
    PuErrorDTO result = ClassificationErrorDTOMapper.map(errorDTO);
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      it.gov.pagopa.pu.classification.dto.generated.ErrorFieldDTO expectedErrorFieldDTO = errorDTO.getFields().get(i);
